...
# --- DYNAMIC PATHS ---
# 1. Get the directory where this script resides (e.g., .../scripts)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# 2. Get the PROJECT ROOT by going up one level from the script folder
#    (e.g., .../video-eeg-electrode-registration)
...
# YOLO Weights (inside the 'runs' folder in project root)
YOLO_WEIGHTS = os.path.join(BASE_DIR, "runs", "detect", "train4", "weights", "best.pt")

# SAM2 Checkpoints
SAM2_CHECKPOINT = os.path.join(CHECKPOINT_DIR, "sam2_hiera_large.pt")
...
def main():
    ...
    # Pre-computation (Isolated State)
