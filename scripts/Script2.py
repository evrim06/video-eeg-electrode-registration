...
def estimate_inion_3d(nas, lpa, rpa):
    ...
    Anatomical facts:
    - INION is on the back of the skull, opposite to NAS
    - NAS-INION distance is typically ~35-38cm (350-380mm) for adults
    - Ear-to-ear (LPA-RPA) is typically ~14-16cm (140-160mm)
    - Ratio of NAS-INION to ear-ear is roughly 2.3-2.5
    - INION is typically slightly BELOW the ear level (negative Z offset)
    
    We estimate INION as:
    - On the opposite side of origin from NAS
    - At a distance proportional to ear-to-ear distance
    - Slightly below the ear-NAS plane
    """
    origin = (lpa + rpa) / 2.0
    ...
    # Up direction (perpendicular to ear axis and forward)
    up = np.cross(ear_axis, forward)
    up = up / np.linalg.norm(up)
    
    # INION distance from origin is typically similar to NAS distance
    # but can be slightly more (head is not perfectly spherical)
    # Using ratio based on anatomical measurements
    inion_distance = forward_len * 1.05  # INION slightly further than NAS
    
    # INION is also slightly below the NAS-ear plane
    # Typically ~10-15mm below for average adult head
    # Scale this relative to ear distance
    z_offset = -0.08 * ear_len  # Small downward offset
...
def main():
    ...
    # STEP 5: TRANSFORM TO HEAD COORDINATES (with uncertainty)
    print("\n--- Step 5: Transforming to mm (with uncertainty) ---")
    ...
    # Estimate uncertainty from observation spread (if we have tracking data)
    if tracking is not None:
        ...
        for obj_id in positions_3d:
            ...
            if len(observations) >= 2:
                ...
                # Default uncertainty if not enough observations
                uncertainties[obj_id] = np.array([5.0, 5.0, 5.0])  # 5mm default
    ...
    for obj_id, pos in final_points.items():
        ...
        if obj_id == "INION":
            ...
            output["landmarks"]["NAS"] = {"position": pos_list, "uncertainty": unc_list}
        elif obj_id == LANDMARK_LPA:
            output["landmarks"]["LPA"] = {"position": pos_list, "uncertainty": unc_list}
        elif obj_id == LANDMARK_RPA:
            output["landmarks"]["RPA"] = {"position": pos_list, "uncertainty": unc_list}
        elif isinstance(obj_id, int) and obj_id >= ELECTRODE_START_ID:
            output["electrodes"][f"E{obj_id - ELECTRODE_START_ID}"] = {
                "position": pos_list,
    ...
    for obj_id, pos in final_points.items():
        ...
        # pos is numpy array from final_points dict
